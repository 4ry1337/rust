# Shareable Mutable Containers

Sometimes multiple references to an object must still mutate it — the [[8. pointers and borrowing#Rules of References|exclusivity rule]] is too strict for this.

*Shareable mutable containers* permit controlled mutability despite aliasing.

- Multi-threaded: [[20. synchronization#Mutex|Mutex<T>]], [[20. synchronization#RwLock|RwLock<T>]], [[20. synchronization#OnceLock|OnceLock<T>]], [[20. synchronization#Atomic types|atomic types]]
- Single-threaded (no `Sync`): `Cell<T>`, `RefCell<T>`, `OnceCell<T>` — provide *interior mutability*

^interior-mutability

## Cell

- Implements interior mutability by moving values in/out — `&T` to the inner value cannot be obtained.
- Value cannot be directly obtained without replacing it with something else.

Methods:
- `get` — retrieves current value by copy (requires `T: Copy`)
- `take` — replaces current value with `Default::default()`, returns replaced value (requires `T: Default`)
- `replace` — replaces current value, returns replaced value
- `into_inner` — consumes the `Cell<T>`, returns interior value
- `set` — replaces interior value, drops the replaced value

> [!EXAMPLE]
> ```rust
> use std::cell::Cell;
> 
> fn main() {
>     let c = Cell::new(5);
>     let old = c.get();            // copy out: 5
>     c.set(10);                    // replace, drop old value
>     let replaced = c.replace(20); // returns 10, cell now holds 20
> 
>     println!("{old} {replaced} {}", c.get()); // 5 10 20
> 
>     let d: Cell<String> = Cell::new(String::from("hello"));
>     let taken = d.take();     // replaces with String::default() (""), returns "hello"
>     println!("{taken} / {}", d.into_inner()); // hello / (empty)
> }
> ```

Preferred over `RefCell` for small, cheap-to-copy types (e.g. numbers). `RefCell` better for larger/non-`Copy` types.

## RefCell

- Implements "dynamic borrowing" — temporary, exclusive, mutable access via lifetimes.
- Borrows tracked at runtime, unlike native references (tracked statically at compile time).

Access:
- `borrow` — obtains `&T`
- `borrow_mut` — obtains `&mut T`
- Both verify borrow rules at call time: any number of immutable borrows, OR a single mutable borrow, never both.
- Violating the rule → thread panics.

> [!EXAMPLE]
> ```rust
> use std::cell::RefCell;
> 
> fn main() {
>     let c = RefCell::new(5);
>     {
>         let r1 = c.borrow();
>         let r2 = c.borrow(); // multiple immutable borrows OK
>         println!("{} {}", *r1, *r2);
>     } // borrows dropped here
> 
>     {
>         let mut m = c.borrow_mut();
>         *m += 10;
>     } // mutable borrow dropped here
> 
>     println!("{}", c.borrow()); // 15
> 
>     // let a = c.borrow();
>     // let b = c.borrow_mut(); // would panic: already borrowed
> }
> ```

`Sync` equivalent: [[20. synchronization#RwLock|RwLock<T>]].

## When to Choose Interior Mutability

Inherited mutability (unique access required to mutate) is preferred — it's what lets Rust statically reason about aliasing. Interior mutability is a last resort, appropriate when:

1. **Introducing mutability inside something immutable** — shared smart pointers (`Rc<T>`, `Arc<T>`) only allow `&` access to contents (multiply-aliased). Wrapping the value in `RefCell<T>` reintroduces mutation. `RefCell` = single-threaded only; use `Mutex`/`RwLock` for shared mutability across threads.
2. **Hiding mutation inside logically-immutable methods** — e.g. a cache populated lazily inside a `&self` method (`OnceCell`).
3. **Mutating `Clone` implementations** — `clone(&self)` can't take `&mut self`, so any internal mutation (e.g. `Rc<T>` bumping its refcount) must go through a cell type.

> [!EXAMPLE]
> ```rust
> use std::cell::{Cell, RefCell, OnceCell};
> use std::rc::Rc;
> 
> fn main() {
>     // 1. Mutability inside immutable Rc
>     let shared = Rc::new(RefCell::new(0));
>     *shared.borrow_mut() += 5;
>     println!("{}", shared.borrow());
> 
>     // 2. Mutation hidden in a &self method (lazy cache)
>     let cache = OnceCell::new();
>     let v = cache.get_or_init(|| 42);
>     println!("{v}");
> 
>     // 3. Mutating Clone via Cell
>     let count = Cell::new(1);
>     count.set(count.get() + 1); // simulates refcount bump on clone
>     println!("{}", count.get());
> }
> ```

[^docs]: https://doc.rust-lang.org/std/cell/index.html
[^book-cell]: https://doc.rust-lang.org/core/cell/index.html
